"""Sweep experiment for different window sizes."""
...
from emg_sampling.paths import WINDOW_SWEEP_CSV, ensure_project_dirs
...
def run_window_sweep(
    ...
    """Runs LDA sweep for provided window sizes and saves table."""
    ensure_project_dirs()
    ...
    out_df = pd.DataFrame(rows).sort_values("win_ms").reset_index(drop=True)
    out_df.to_csv(WINDOW_SWEEP_CSV, index=False)
    return out_df
